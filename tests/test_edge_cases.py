...
def test_identical_primitives_lists_and_dicts_score_one():
    assert ObjectAligner("p", {"type": "string", "score": "exact"}).metric("x", "x")["score"] == 1.0
    assert ObjectAligner("l", {"type": "array", "items": {"type": "string", "score": "exact"}, "order": "fixed"}).metric(["x"], ["x"])["score"] == 1.0
    assert ObjectAligner(
        "d",
        {"type": "object", "properties": {"x": {"type": "string", "score": "exact"}}, "keyScore": "exact"},
    ).metric({"x": "y"}, {"x": "y"})["score"] == 1.0


def test_completely_disjoint_primitives_lists_and_dicts_score_zero():
    assert ObjectAligner("p", {"type": "string", "score": "exact"}).metric("x", "y")["score"] == 0.0
    assert ObjectAligner("l", {"type": "array", "items": {"type": "string", "score": "exact"}, "order": "align"}).metric(["x"], ["y"])["score"] == 0.0
    assert ObjectAligner(
        "d",
        {"type": "object", "properties": {"a": {"type": "integer", "score": "exact"}}, "keyScore": "exact"},
    ).align({"a": 1}, {"b": 2}, skip_validation=True).score == 0.0


def test_single_element_lists_and_single_key_dicts():
    fixed = ObjectAligner("fixed", {"type": "array", "items": {"type": "string", "score": "exact"}, "order": "fixed"})
    reorder = ObjectAligner("reorder", {"type": "array", "items": {"type": "string", "score": "exact"}, "order": "align"})
    exact_dict = ObjectAligner("exact-dict", {"type": "object", "properties": {"x": {"type": "integer"}}, "keyScore": "exact"})
    fuzzy_dict = ObjectAligner("fuzzy-dict", {"type": "object", "properties": {"x": {"type": "integer"}}, "keyScore": "jaro", "keyThreshold": 0.0})
...
def test_empty_unicode_and_emoji_strings():
    exact = ObjectAligner("exact", {"type": "string", "score": "exact"})
    jaro = ObjectAligner("jaro", {"type": "string", "score": "jaro"})
    emoji = ObjectAligner("emoji", {"type": "object", "properties": {"name": {"type": "string", "score": "exact"}}, "keyScore": "exact"})
...
def test_float_precision_mixed_number_types_and_integer_validation():
    number = ObjectAligner("number", {"type": "number", "score": "invdiff"})
    integer = ObjectAligner("integer", {"type": "integer", "score": "invdiff"})
