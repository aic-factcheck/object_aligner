...
- Metric callable contract:
  ...
  - **Context-aware opt-in:** a metric carrying `wants_context = True` (set via the public `context_metric` decorator) is instead called `(gold, pred, context) -> float`, where `context` is the public frozen `ScoreContext(gold_parent, pred_parent, gold_root, pred_root, path)`. Detection is `getattr(scoref, "wants_context", False)` at the single call site in `_align_primitive` (`_aligner_core.py`); plain 2-arg metrics are byte-identical and pay nothing (the context object is built lazily). Roots live on `_AlignContext.gold_root/pred_root` (set in `_align_with_ctx`); `gold_parent`/`pred_parent`/`path` are threaded as keyword-only params through `_align_helper` and the container aligners, with the invariant "a container passes *itself* as its children's parent" (so the n×m reorder cost matrix hands each cell its own cross-paired parent). `_align_lists` passes the *original* (unsliced) list as parent and an `index_offset` so combined prefix+items paths stay absolute. The referential id-derivation call (`_aligner_referential.py:_derive_single_scope`) passes the definer item lists as parents to avoid the `except (...): cost=0.0` swallowing a `None`-parent dereference. `ScoreContext` holds live references (read-only by contract, not copied — mutation breaks determinism). `context_metric` / `ScoreContext` are re-exported from the package root. Booleans stay exact-only (no context). See `docs/primitives.md` "Context-aware custom metrics".
