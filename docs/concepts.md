...
The main entry point. You create it with a schema and optional description configuration:

```python
from object_aligner import ObjectAligner

aligner = ObjectAligner(schema)
aligner_with_description = ObjectAligner(schema, generate_description=True)
```

`description_templates` (and `feedback_templates`) let you override selected built-in template strings. The packaged defaults ship as TOML data under `src/object_aligner/templates/` (`describe.toml`, `feedback.toml`, `feedback.compact.toml`); the public helper `load_templates_from_toml(path)` reads user-authored override files in the same format.
...
- **`metric(gold, pred, debug=False, generate_description=None, generate_feedback=None)`** — Validates both objects against the schema, runs alignment, and returns `{"score": ...}` by default. It adds `"description"` when description is enabled (a string, or a structured dict when `generate_description="full"`), a top-K prescriptive `"feedback"` string when `generate_feedback=True` (or a structured dict when `generate_feedback="full"`), and a structured `"debug"` alignment tree when `debug=True`.
